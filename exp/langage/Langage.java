...
/**
 * The interpreter: Implement the loop for parsing instructions it is a Thread
 * that loop until the quit instruction I choose to make it a thread to separate
 * the input method from the parsing
 * 
 * @see exp.exp.interprete.Test
 * @author pfares
 */
public class Langage extends Thread {
	...
	// des affectations
	/**
	 * ts : est l'environnement dynamique attribut hérité de L vers LE
	 */
	...
	public void run() {
		...
		while ((mt.getTok()) != Token.QUIT) {
			try {
				System.out.println(Instruction.parse(mt, ts));
				if (mt.getTok() != Token.FININST) {
					throw new LangageMathException(
							"erreur!! pas de fin inst");
				} else {
					mt.nToken();
				}
			} catch (LangageMathException e) {
