...
public class ExpB extends Exp {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1504002539824890581L;
	int op;
	Exp expg;
	Exp expd;
	...
	public Exp plus(Exp e1, Exp e2) {
		if ((e1 instanceof Const) && (e2 instanceof Const)) {
			return new Const(getEnv(), ((Const) e1).getValeur()
					+ ((Const) e2).getValeur());
		} else
			return new ExpB(getEnv(), Operateur.PLUS, e1, e2);
	}
	...
	public Exp moins(Exp e1, Exp e2) {
		if ((e1 instanceof Const) && (e2 instanceof Const)) {
			...
			return new ExpB(getEnv(), Operateur.MOINS, e1, e2);
	}
	...
	public Exp mult(Exp e1, Exp e2) {
		if ((e1 instanceof Const) && (e2 instanceof Const)) {
			...
			return new ExpB(getEnv(), Operateur.MULT, e1, e2);
	}
	...
	public Exp div(Exp e1, Exp e2) {
		if ((e1 instanceof Const) && (e2 instanceof Const)) {
			...
			return new ExpB(getEnv(), Operateur.DIV, e1, e2);
	}

	public Exp max(Exp e1, Exp e2) {
		if ((e1 instanceof Const) && (e2 instanceof Const)) {
			return new Const(getEnv(), java.lang.Math.max(
					((Const) e1).getValeur(), ((Const) e2).getValeur()));
		} else if (e1 instanceof Const)
			return e1;
		else if (e2 instanceof Const)
			return e2;
		else
			return new ExpB(getEnv(), Operateur.MAX, e1, e2);
	}

	public Exp min(Exp e1, Exp e2) {
		if ((e1 instanceof Const) && (e2 instanceof Const)) {
			return new Const(getEnv(), java.lang.Math.min(
					((Const) e1).getValeur(), ((Const) e2).getValeur()));
		} else if (e1 instanceof Const)
			return e1;
		else if (e2 instanceof Const)
			...
			return new ExpB(getEnv(), Operateur.MIN, e1, e2);
	}
	...
	public Exp eval() {
		...
		case Operateur.PLUS:
			return (plus(expg.eval(), expd.eval()));
		case Operateur.MOINS:
			return (moins(expg.eval(), expd.eval()));
		case Operateur.MULT:
			return (mult(expg.eval(), expd.eval()));
		case Operateur.DIV:
			return div(expg.eval(), expd.eval());
		case Operateur.MAX:
			return max(expg.eval(), expd.eval());
		case Operateur.MIN:
			return min(expg.eval(), expd.eval());
		default:
			return null;
		}
	}
	...
	private void parseOperateur(NaopTokenizer mt)
			throws ExpBException {

		if (mt.getTok() == Token.PLUS)
			op = Operateur.PLUS;
		else if (mt.getTok() == Token.MULT)
			op = Operateur.MULT;
		else if (mt.getTok() == Token.MOINS)
			op = Operateur.MOINS;
		else if (mt.getTok() == Token.DIV)
			op = Operateur.DIV;
		else if (mt.getTok() == Token.MAX)
			op = Operateur.MAX;
		else if (mt.getTok() == Token.MIN)
			op = Operateur.MIN;
		else
			throw new ExpBException("N'est pas un operateur");
		mt.nToken(); // skip l'op�rateur
	}

	/**
	 * Le parser par descente récursive et grammaire denotationelle Les
	 * attributs: Table symbole : hérité L'objet généré Exp synthétisé
	 * 
	 * @param mt
	 *            Tokenizer courant
	 * @param ts
	 *            La table de symbole attribue hérité
	 * @throws ExpException
	 *             Les erreur d'interprétation
	 * @return Le modèle de l'expression
	 */
	public static Exp parse(NaopTokenizer mt,
			Environnement ts) throws ExpException {
