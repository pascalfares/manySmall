...
/**
 * EXP ::= CONST | VAR | EXPB | PREVIOUS | LAMBDA | CONDITION | BIND EXPB ::=
 * (EXP OP EXP) ExpU ::= OPU EXP OP ::= + | - | * | / OPU ::= #
 */
public abstract class Exp implements java.io.Serializable {
	 ...
	 * 
	 * @param e
	 *            The envirenment of the expression
	 */
	...
	/**
	 * abstract method for the semantic of the expression Const => Const Var =>
	 * Eval definition or unchanged exp op exp si exp1 et exp2 Const réduction à
	 * un seul Const
	 * 
	...
	/**
	 * the parser for recursive descent and attributed grammar the attributess:
	 * symbol table : herited the générated expression : synthetised
	 * 
	 * @param mt
	 *            Tokenizer courant
	 * @param ts
	...
	public static Exp parse(NaopTokenizer mt,
			exp.langage.Environnement ts) throws exp.exceptions.ExpException {
		if (mt.getTok() == Token.PO)
			return ExpB.parse(mt, ts);
		else if (mt.getTok() == Token.CONST)
			return Const.parse(mt, ts);
		else if (mt.getTok() == Token.SYMBOL)
			return Var.parse(mt, ts);
		throw (new exp.exceptions.ExpException(
				"N'est pas un debut d'expression"));
	}
