...
public class NewSimpleFilter implements Filter {
	...
	*/
	...
	Throwable problem = null;
	    ...
	    try {
            System.out.println("new filter ----------------------------------------------------le debut des appels");
            //response.getWriter().println("Pas disponble ... revenir plus tard");

            HttpServletRequest httprequest = (HttpServletRequest) request;
            HttpServletResponse httpresponse = (HttpServletResponse) response;
            //do this only when request is not for login.jsp
            if (httprequest.getRequestURI().indexOf("index.jsp") == -1) {
                HttpSession session = httprequest.getSession();
                Parametres param = (Parametres) session.getAttribute("b");
                if (param != null) {
                    if (param.getNo() == 0) {
                        httpresponse.sendRedirect("/informationsTest/index.jsp");
                        return;
                    }
                } else {//si c'est la premier session
                    httpresponse.sendRedirect("/informationsTest/index.jsp");
                    return;
                }
